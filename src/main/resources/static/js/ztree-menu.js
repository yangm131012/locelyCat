...
function initMenuDatas(roleId, idName){
	$.ajax({
		type : 'post',
		url : '/permissions/seletById?roleId=' + roleId,
		success : function(data) {
			var length = data.length;
			var ids = [];
			for(var i=0; i<length; i++){
			...
			initMenuCheck(ids, idName);
		}
	});
}

function initMenuCheck(ids, idName) {
...
function getCheckedMenuIds(idName){
	var treeObj = $.fn.zTree.getZTreeObj(idName);
